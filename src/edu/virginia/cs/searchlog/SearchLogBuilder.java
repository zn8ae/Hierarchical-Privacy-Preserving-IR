...
public class SearchLogBuilder {
    ...
    private final HashMap<String, ArrayList<String>> searchLog;
    ...
    private void sortByComparator(HashMap<String, Integer> unsortedMap, boolean order, int skip, int top) throws Throwable {
        ...
        for (Map.Entry<String, Integer> entry : list) {
            if (i < (skip + 50)) {
                ...
                i++;
                ArrayList<String> record = searchLog.get(entry.getKey());
     ...
     * @param content
     */
    private void generateSearchLog(ArrayList<String> lines) {
        boolean flag = true;
        for (String line : lines) {
            ...
            if (words.length >= 2) {
                ...
                if (!user_id.isEmpty() && !user_query.isEmpty()) {
                    ...
                    if (searchLog.containsKey(user_id)) {
                        temp = searchLog.get(user_id);
                    } else {
                        temp = new ArrayList<>();
                    }
                    String clicked_url;
    ...
    private void LoadDirectory(String folder, int choice) throws Throwable {
        ...
        for (File f : dir.listFiles()) {
            if (f.isFile()) {
                ...
                if (choice == 1) {
                    generateSearchLog(fiop.LoadFile(f.getAbsolutePath(), -1));
                } else if (choice == 2) {
        ...
        if (choice == 1) {
            System.out.println("Total Number of Unique Profiles - " + searchLog.size());
        }
    }
     ...
     * @throws Throwable
     */
    private void generateTopSearchLogs(int skip, int top) throws Throwable {
        HashMap<String, Integer> tempMap = new HashMap<>();
        for (Map.Entry<String, ArrayList<String>> entry : searchLog.entrySet()) {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        pBuilder.generateTopSearchLogs(0, 1000);
    }
